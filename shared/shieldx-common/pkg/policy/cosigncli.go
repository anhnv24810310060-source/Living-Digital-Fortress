...
func (c CosignCLI) Sign(digest string, out io.Writer) error {
    ...
    if _, err := blob.WriteString(digest); err != nil { blob.Close(); return err }
    blob.Close()
    ...
    if c.KeyRef != "" { args = append(args, "--key", c.KeyRef) }
...
func (c CosignCLI) Verify(digest string, sig io.Reader) error {
    ...
    if _, err := io.Copy(sigFile, sig); err != nil { sigFile.Close(); return err }
    sigFile.Close()
