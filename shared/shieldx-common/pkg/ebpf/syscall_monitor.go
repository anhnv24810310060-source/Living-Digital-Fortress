...
func (sm *SyscallMonitor) CalculateThreatScore() int {
	...
	// Weight factors tuned to align with behavioral expectations
	weights := map[string]float64{
		"dangerous_ratio":       70.0,
		"unusual_patterns":      25.0,
		"shell_execution":       15.0,
		"rapid_process_spawn":   10.0,
		"high_network_activity": 10.0,
	}

	// Dangerous syscall prevalence has the strongest influence on the score
	if features.EventCount > 0 {
		dangerousRatio := float64(features.DangerousSyscalls) / float64(features.EventCount)
		score += dangerousRatio * weights["dangerous_ratio"]

		switch {
		case features.DangerousSyscalls >= 40:
			score += 30
		case features.DangerousSyscalls >= 20:
			score += 20
		case features.DangerousSyscalls >= 10:
			score += 10
		}
	}

	// Unusual patterns scale linearly up to five distinct detections
	if features.UnusualPatterns > 0 {
		score += float64(min(features.UnusualPatterns, 5)) * (weights["unusual_patterns"] / 5.0)
	}

	// Shell execution is a strong escalation indicator
	if features.ShellExecution > 0 {
		score += weights["shell_execution"]
	}

	// Sustained process spawning adds marginal risk
	if features.ProcessCalls > 10 {
		score += weights["rapid_process_spawn"]
	}

	// Heavy network activity contributes to the threat posture
