...
func TestMutualTLSWithSPIFFE(t *testing.T) {
    ...
    if err != nil { t.Fatalf("issuer: %v", err) }
    defer issuer.Stop()
...
func TestRotationHappens(t *testing.T) {
    ...
    if err != nil { t.Fatalf("get leaf: %v", err) }
    // Wait for rotation
...
func (ai *AutoIssuer) GetLeafForTest() (tls.Certificate, error) {
    ...
    if len(ai.currentLeaf.Certificate) == 0 { return tls.Certificate{}, nil }
    return ai.currentLeaf, nil
}

func tlsCertEqual(a, b tls.Certificate) bool {
    if len(a.Certificate) == 0 || len(b.Certificate) == 0 { return false }
    if len(a.Certificate[0]) != len(b.Certificate[0]) { return false }
    // compare first raw cert bytes
    for i := range a.Certificate[0] {
        if a.Certificate[0][i] != b.Certificate[0][i] { return false }
    }
    return true
}