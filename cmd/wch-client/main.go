...
func loadConfig(path string) *Config {
    ...
    var c Config
    if err := json.Unmarshal(b, &c); err != nil { return nil }
    return &c
}
...
func main() {
    ...
    if c := loadConfig(*cfgPath); c != nil {
        if c.Locator != "" { *locator = c.Locator }
        if c.Ingress != "" { *ingress = c.Ingress }
        if c.UseQUIC { *useQUIC = true }
        if c.QuicAddr != "" { *quicAddr = c.QuicAddr }
        if c.Tenant != "" { *tenant = c.Tenant }
        if c.Scope != "" { *scope = c.Scope }
        if c.Path != "" { *path = c.Path }
        if c.UDPTarget != "" { *udpTarget = c.UDPTarget }
        if c.UDPHex != "" { *udpHex = c.UDPHex }
        if c.RebindMs > 0 { *rebindMs = c.RebindMs }
        // Prefer first ingress in list if provided
        if len(c.IngressList) > 0 { *ingress = c.IngressList[0] }
    }

    var qconn quic.Connection
    var qconf = &quic.Config{EnableDatagrams: true, KeepAlivePeriod: time.Duration(*kaSec) * time.Second, MaxIdleTimeout: time.Duration(*idleSec) * time.Second, TokenStore: quic.NewLRUTokenStore(64), Allow0RTT: true}
    if *useQUIC {
        tlsConf := &tls.Config{InsecureSkipVerify: true, NextProtos: []string{"shieldx-wch"}}
        c, err := quic.DialAddrEarly(*quicAddr, tlsConf, qconf)
        if err == nil { qconn = c }
        defer func(){ if qconn != nil { qconn.CloseWithError(0, "bye") } }()
        startPing()
    }

    ensureQUICConn := func() error {
        if !*useQUIC { return nil }
        if qconn != nil { return nil }
        tlsConf := &tls.Config{InsecureSkipVerify: true, NextProtos: []string{"shieldx-wch"}}
        var lastErr error
        for i:=0;i<5;i++{
            c, err := quic.DialAddrEarly(*quicAddr, tlsConf, qconf)
            if err == nil { qconn = c; return nil }
            lastErr = err
    ...
    startPing := func() {
        ...
        go func(){
            ...
            for {
                ...
                case <-t.C:
                    if qconn == nil { continue }
                    _ = qconn.SendMessage([]byte(`{"ping":true}`))
                }
            }
        }()
    }
    ...
    if c := loadConfig(*cfgPath); c != nil && len(c.IngressList) > 0 { ingresses = c.IngressList }
    for _, base := range ingresses {
        connResp = postJSONMay(base+"/connect", map[string]any{"token": token})
        if connResp != nil { *ingress = base; break }
    }
    if connResp == nil { log.Fatalf("connect failed on all ingresses") }
    channelID := connResp["channelId"].(string)
    guardianPubB64 := connResp["guardianPubKey"].(string)
    if *rebindMs == 0 {
        if v, ok := connResp["rebindHintMs"]; ok {
            if f, ok2 := v.(float64); ok2 { *rebindMs = int(f) }
        }
    }
    ...
    env := map[string]any{
        ...
        if *udpTarget != "" {
            ...
            if qconn != nil || ensureQUICConn() == nil {
                nr, cr, err := sendQUICDatagram(qconn, b)
                if err != nil {
                    log.Printf("datagram send failed: %v, retrying...", err)
                    if qconn != nil { qconn.CloseWithError(0, "retry"); qconn = nil }
                    if ensureQUICConn() == nil {
                        nr, cr, err = sendQUICDatagram(qconn, b)
                    }
                }
                if err != nil { log.Printf("retry failed, fallback stream"); nr, cr, err = sendQUIC(*quicAddr, b); if err != nil { log.Fatalf("QUIC send: %v", err) } }
                nonce2b64 = nr
                ct2b64 = cr
            } else {
                nr, cr, err := sendQUIC(*quicAddr, b)
                if err != nil { log.Fatalf("QUIC send: %v", err) }
                nonce2b64 = nr
                ct2b64 = cr
            }
        } else {
    ...
    if err := json.Unmarshal(pt2, &innerResp); err != nil { panic(err) }
    fmt.Printf("Status: %d\n", innerResp.Status)
    fmt.Printf("Body  : %s\n", string(innerResp.Body))
        } else {
            var udpResp wch.UDPResponse
            if err := json.Unmarshal(pt2, &udpResp); err != nil { panic(err) }
...
func postJSONMust(url string, payload map[string]any) map[string]any {
    ...
    if err := json.NewDecoder(res.Body).Decode(&m); err != nil { panic(err) }
    return m
}
...
func postJSONMay(url string, payload map[string]any) map[string]any {
    ...
    var m map[string]any
    if err := json.NewDecoder(res.Body).Decode(&m); err != nil { return nil }
    return m
}
...
func sendQUIC(addr string, body []byte) (nonceB64 string, ctB64 string, err error) {
    ...
    if err != nil { return "", "", err }
    if _, err := st.Write(body); err != nil { return "", "", err }
    _ = st.Close()
    respBytes, err := io.ReadAll(st)
    if err != nil { return "", "", err }
    var m map[string]string
    if err := json.Unmarshal(respBytes, &m); err != nil { return "", "", err }
    return m["nonce"], m["ciphertext"], nil
}

func sendQUICDatagram(conn quic.Connection, body []byte) (nonceB64 string, ctB64 string, err error) {
    if conn == nil { return "", "", fmt.Errorf("no quic conn") }
    if err := conn.SendMessage(body); err != nil { return "", "", err }
    resp, err := conn.ReceiveMessage()
    if err != nil { return "", "", err }
    var m map[string]string
    if err := json.Unmarshal(resp, &m); err != nil { return "", "", err }
    return m["nonce"], m["ciphertext"], nil
}
