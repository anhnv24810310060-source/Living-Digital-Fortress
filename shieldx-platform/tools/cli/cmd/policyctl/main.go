...
func main() {
    ...
    case "sign":
        ...
        if err != nil { log.Fatal(err) }
        if err := os.MkdirAll(filepath.Dir(*out), 0o755); err != nil { log.Fatal(err) }
        if err := os.WriteFile(*out, sig, 0o644); err != nil { log.Fatal(err) }
    ...
    case "verify":
        ...
        if err != nil { log.Fatal(err) }
        if err := policy.VerifyDigest(policy.NoopVerifier{}, digest, sig); err != nil { log.Fatal(err) }
