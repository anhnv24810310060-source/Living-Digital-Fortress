...
func VerifyEdDSA(dpopJWT string, method string, htu string, now time.Time, skewSec int64) (string, string, int64, error) {
    ...
    if len(parts) != 3 { return "", "", 0, errors.New("invalid dpop format") }
    ...
    if err := json.Unmarshal(headerJSON, &hdr); err != nil { return "", "", 0, err }
    if alg, _ := hdr["alg"].(string); alg != "EdDSA" { return "", "", 0, errors.New("unsupported alg") }
    jwk, ok := hdr["jwk"].(map[string]any)
    if !ok { return "", "", 0, errors.New("missing jwk") }
    if kty, _ := jwk["kty"].(string); kty != "OKP" { return "", "", 0, errors.New("unsupported kty") }
    if crv, _ := jwk["crv"].(string); crv != "Ed25519" { return "", "", 0, errors.New("unsupported crv") }
    xstr, _ := jwk["x"].(string)
    x, err := b64url.DecodeString(xstr)
    if err != nil { return "", "", 0, fmt.Errorf("jwk x: %w", err) }
    if len(x) != ed25519.PublicKeySize { return "", "", 0, errors.New("bad jwk x size") }
    ...
    if err := json.Unmarshal(payloadJSON, &pl); err != nil { return "", "", 0, err }
    htm, _ := pl["htm"].(string)
    htuPl, _ := pl["htu"].(string)
    jti, _ := pl["jti"].(string)
    iatFloat, _ := pl["iat"].(float64)
    iat := int64(iatFloat)
    if strings.ToUpper(htm) != strings.ToUpper(method) { return "", "", 0, errors.New("htm mismatch") }
    if htuPl != htu { return "", "", 0, errors.New("htu mismatch") }
    if jti == "" { return "", "", 0, errors.New("missing jti") }
    // iat skew check
    nowSec := now.Unix()
    if iat > nowSec+skewSec || iat < nowSec-skewSec { return "", "", 0, errors.New("iat out of range") }
    // Verify signature on header.payload
    signing := []byte(parts[0] + "." + parts[1])
    if !ed25519.Verify(pub, signing, sig) { return "", "", 0, errors.New("dpop signature invalid") }
