...
func (m *manager) EnsureDevice(iface string, privKeyB64 string, listenPort int) error {
	...
	if privKeyB64 != "" {
		...
		if len(pkRaw) != 32 {
			return errors.New("invalid wg private key length")
		}
...
func (m *manager) AddPeer(iface string, clientPubKeyB64 string, allowedCIDR string, endpoint string, persistentKeepalive int, ttl time.Duration) error {
	...
	if len(pubRaw) != 32 {
		return errors.New("invalid wg public key length")
	}
	...
	peer := wgtypes.PeerConfig{
		...
		PersistentKeepaliveInterval: func() *time.Duration {
			...
			d := time.Duration(ka) * time.Second
			return &d
		}(),
		Endpoint: ep,
	}
...
func (m *manager) ListPeers(iface string) ([]PeerInfo, error) {
	...
	for _, p := range dev.Peers {
		...
		for _, ipn := range p.AllowedIPs {
			pi.AllowedIPs = append(pi.AllowedIPs, ipn.String())
		}
