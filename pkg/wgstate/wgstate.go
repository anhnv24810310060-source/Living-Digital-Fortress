...
func (st *Store) SetServer(priv, pub string) {
	st.mu.Lock()
	st.s.ServerPrivB64, st.s.ServerPubB64 = priv, pub
	st.mu.Unlock()
}
func (st *Store) SetEndpoint(endpoint string) {
	st.mu.Lock()
	st.s.PublicEndpoint = endpoint
	st.mu.Unlock()
}
func (st *Store) SetHealthURL(url string) { st.mu.Lock(); st.s.PublicHealthURL = url; st.mu.Unlock() }
func (st *Store) SetAltServer(priv, pub string, port int) {
	st.mu.Lock()
	st.s.AltServerPrivB64, st.s.AltServerPubB64, st.s.AltListenPort = priv, pub, port
	st.mu.Unlock()
}
func (st *Store) ClearAltServer() {
	st.mu.Lock()
	st.s.AltServerPrivB64, st.s.AltServerPubB64, st.s.AltListenPort = "", "", 0
	st.mu.Unlock()
}

func (st *Store) AddPeer(p Peer) { st.mu.Lock(); st.s.Peers[p.ClientPub] = p; st.mu.Unlock() }

func (st *Store) RemovePeer(clientPub string) {
	st.mu.Lock()
	delete(st.s.Peers, clientPub)
	st.mu.Unlock()
}
...
func (st *Store) AllocateIP(clientPub string) string {
	st.mu.Lock()
	defer st.mu.Unlock()
	if p, ok := st.s.Peers[clientPub]; ok && p.AssignedIP != "" {
		return p.AssignedIP
	}
	st.s.LastIPOctet++
	if st.s.LastIPOctet < 2 {
		st.s.LastIPOctet = 2
	}
	if st.s.LastIPOctet > 254 {
		st.s.LastIPOctet = 2
	}
	ip := "10.10.0." + itoa(st.s.LastIPOctet) + "/32"
	if p, ok := st.s.Peers[clientPub]; ok {
		p.AssignedIP = ip
		st.s.Peers[clientPub] = p
	}
	return ip
}
