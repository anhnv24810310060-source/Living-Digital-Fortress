...
type SandboxResult struct {
	...
	ThreatScore  float64           `json:"threat_score"` // 0-100 range
	RiskLevel    string            `json:"risk_level"`   // LOW/MEDIUM/HIGH/CRITICAL
	
	// eBPF monitoring data
	Syscalls     []ebpf.SyscallEvent `json:"syscalls"`
	...
	// Sandbox metadata
	Backend      string              `json:"backend"` // "firecracker"
	IsolationLevel string            `json:"isolation_level"`
}

// FileAccessEvent tracks file operations with threat context
type FileAccessEvent struct {
	Timestamp  time.Time
	Path       string
	Operation  string // read/write/delete/chmod
	Success    bool
	Suspicious bool   // Flagged by heuristics
}
...
type NetworkEvent struct {
	...
	Destination string
	BytesSent   int64
	Blocked     bool // Should always be true
}
...
func (fr *FirecrackerRunner) Run(ctx context.Context, payload string) (*SandboxResult, error) {
	...
	// Step 6: Advanced threat scoring (P0: must be 0-100)
	...
	// Step 8: Extract syscall events from monitor
	...
	// Step 9: Store forensic artifacts (hashed, never raw payload)
	result.Artifacts["payload_sha256"] = []byte(hashPayload(payload))
	result.Artifacts["threat_explanation"] = []byte(explanation)
	result.Artifacts["ebpf_features"] = marshalFeatures(features)
	
	// Step 10: ML features for behavioral analysis
...
func (fr *FirecrackerRunner) extractSyscallEvents(monitor *ebpf.SyscallMonitor) []ebpf.SyscallEvent {
	features := monitor.ExtractFeatures()
	events := make([]ebpf.SyscallEvent, 0, len(features.SyscallSequence))
	
	for i, syscall := range features.SyscallSequence {
		if syscall == "" {
			continue
		}
		events = append(events, ebpf.SyscallEvent{
...
// RiskLevel maps threat score to risk category
