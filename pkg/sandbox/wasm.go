...
func (w *WASMRunner) Run(ctx context.Context, payload string) (string, error) {
	if len(w.moduleBytes) == 0 {
		return "", errors.New("empty wasm module")
	}
	...
	if fn == nil {
		return "", fmt.Errorf("exported function not found: %s", w.funcName)
	}
	// Pass payload via memory: write bytes and call
	mem := mod.Memory()
	if mem == nil {
		return "", errors.New("no memory in module")
	}
	// Very simple ABI: write payload at offset 0, call(fn, len)
	if ok := mem.Write(0, []byte(payload)); !ok {
		return "", errors.New("mem write failed")
	}
	...
	if !ok {
		return "", errors.New("mem read failed")
	}
