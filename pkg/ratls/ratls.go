...
func (ai *AutoIssuer) newLeaf() (tls.Certificate, error) {
	...
	if err != nil {
		return tls.Certificate{}, fmt.Errorf("gen leaf key: %w", err)
	}
	serial, _ := rand.Int(rand.Reader, big.NewInt(1<<62))
	...
	if err != nil {
		return tls.Certificate{}, fmt.Errorf("create leaf: %w", err)
	}
	leafPEM := pem.EncodeToMemory(&pem.Block{Type: "CERTIFICATE", Bytes: der})
	keyPEM := pem.EncodeToMemory(&pem.Block{Type: "RSA PRIVATE KEY", Bytes: x509.MarshalPKCS1PrivateKey(key)})
	cert, err := tls.X509KeyPair(leafPEM, keyPEM)
	if err != nil {
		return tls.Certificate{}, fmt.Errorf("x509 keypair: %w", err)
	}
	return cert, nil
}
	...
	return &tls.Config{
		...
		VerifyPeerCertificate: func(rawCerts [][]byte, verifiedChains [][]*x509.Certificate) error {
			...
			if expectedTrustDomain != "" {
				...
				if !ok {
					return fmt.Errorf("peer spiffe trust domain mismatch")
				}
			}
			return nil
		},
