...
func LoadFromZipReader(r *zip.Reader) (*Bundle, error) {
    ...
    if mf.Name == "" { return nil, fmt.Errorf("manifest.json not found") }
    ...
    if len(mf.Policies) > 0 {
        ...
        for _, rel := range mf.Policies {
            ...
            if b, ok := files[p]; ok { filtered[p] = b }
        }
        files = filtered
    } else {
        // otherwise, include all .rego files in sorted order
        paths := make([]string, 0, len(files))
        for p := range files { if strings.HasSuffix(p, ".rego") { paths = append(paths, p) } }
        sort.Strings(paths)
        ordered := map[string][]byte{}
        for _, p := range paths { ordered[p] = files[p] }
        files = ordered
    }
