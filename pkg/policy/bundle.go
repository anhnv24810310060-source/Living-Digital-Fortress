...
func (b *Bundle) WriteZip(outPath string) error {
	...
	if _, err := mfw.Write(mfb); err != nil {
		return err
	}
	// files
	paths := make([]string, 0, len(b.Files))
...
type Signer interface {
	Sign(digest string, out io.Writer) error
}
type Verifier interface {
	Verify(digest string, sig io.Reader) error
}

// NoopSigner writes the digest into the signature as plain text (demo only).
type NoopSigner struct{}

func (NoopSigner) Sign(digest string, out io.Writer) error {
	_, err := io.WriteString(out, digest)
	return err
}
...
func BuildAndWrite(dir, outPath string) (string, error) {
	...
	if err := b.WriteZip(outPath); err != nil {
		return "", err
	}
	return b.Hash()
}
...
func SignDigest(s Signer, digest string) ([]byte, error) {
	...
	if err := s.Sign(digest, &buf); err != nil {
		return nil, err
	}
