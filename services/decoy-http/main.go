...
func rootHandler(w http.ResponseWriter, r *http.Request) {
    ...
    if len(banners) == 0 { banners = []string{"nginx/1.23.4"} }
...
func main() {
    ...
    reg.Register(mHits); mux.Handle("/metrics", reg)
...
func currentBanners() []string {
    ...
    if url == "" { return []string{"nginx/1.23.4", "Apache/2.4.57 (Unix)", "Caddy", "envoy"} }
    resp, err := http.Get(url + "/config/banners")
    if err != nil { return []string{"nginx/1.23.4"} }
    defer resp.Body.Close()
    b, _ := io.ReadAll(resp.Body)
    var m struct{ ServerBanners []string `json:"server_banners"` }
    if err := json.Unmarshal(b, &m); err != nil { return []string{"nginx/1.23.4"} }
