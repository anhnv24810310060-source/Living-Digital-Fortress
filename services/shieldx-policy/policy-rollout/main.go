...
func main() {
    addr := ":8099"
    if v := os.Getenv("POLICY_ROLLOUT_PORT"); v != "" { addr = ":"+v }
    st := &state{}
    ...
    if v := os.Getenv("REGISTRY_DIGEST_INTERVAL"); v != "" {
        if d, err := time.ParseDuration(v); err == nil { pollEvery = d }
    }
    ...
    http.HandleFunc("/apply", func(w http.ResponseWriter, r *http.Request) {
        var req struct{ Digest string `json:"digest"`; URL string `json:"url"`; Sig string `json:"sig"` }
        ...
        if req.URL != "" {
            ...
            if verr != nil {
                ...
                http.Error(w, "verify failed: "+verr.Error(), 400); return
            }
        ...
        if req.Digest == "" { http.Error(w, "missing digest or url", 400); return }
        ...
        if err := policy.VerifyDigest(policy.NoopVerifier{}, req.Digest, []byte(req.Digest)); err != nil {
            atomic.AddUint64(&st.verifyFail, 1)
            http.Error(w, "verify failed", 400); return
        }
        atomic.AddUint64(&st.verifyOK, 1)
    ...
    go func() {
        ...
        for range ticker.C {
            ...
            if rand.Intn(100) < 5 { atomic.AddUint64(&st.driftCount, 1) }
...
func fetchAndVerify(url, sigURL string) (string, error) {
    ...
    if resp.StatusCode != 200 { return "", fmt.Errorf("fetch bundle: status %d", resp.StatusCode) }
    ...
    if strings.TrimSpace(sigURL) != "" {
        ...
        if sresp.StatusCode != 200 { return "", fmt.Errorf("fetch sig: status %d", sresp.StatusCode) }
        sig, err := io.ReadAll(sresp.Body)
        if err != nil { return "", err }
        if err := policy.VerifyDigest(policy.CosignCLI{}, digest, sig); err != nil { return "", err }
    }
    return digest, nil
}
