...
type Phase1Enhancement struct {
	...
	// PQC components
	pqcKeypair    *pqcrypto.HybridKeypair
	pqcMu         sync.RWMutex
...
func (p1 *Phase1Enhancement) initPQC() error {
	var kp *pqcrypto.HybridKeypair
	var err error
	
	switch strings.ToLower(p1.config.PQCAlgorithm) {
	case "kyber1024":
		kp, err = pqcrypto.GenerateHybridKeypair()
	case "dilithium5":
		kp, err = pqcrypto.GenerateHybridKeypair()
	case "hybrid":
		kp, err = pqcrypto.GenerateHybridKeypair()
	...
	p1.pqcKeypair = kp
	log.Printf("[phase1] PQC initialized with algorithm: %s", p1.config.PQCAlgorithm)
...
func (p1 *Phase1Enhancement) rotatePQCKeys() error {
	newKp, err := pqcrypto.GenerateHybridKeypair()
	if err != nil {
		return err
	}
	...
	_ = ledger.AppendJSONLine(ledgerPath, serviceName, "pqc.rotation", map[string]any{
		...
		"old_kyber_pub": oldKp.Kyber.PublicKey[:32],
		"new_kyber_pub": newKp.Kyber.PublicKey[:32],
	})
...
func (p1 *Phase1Enhancement) GetPQCPublicKey() ([]byte, error) {
	...
	if p1.pqcKeypair == nil {
		return nil, fmt.Errorf("PQC not initialized")
	}
	
	return p1.pqcKeypair.Kyber.PublicKey, nil
}
...
func (p1 *Phase1Enhancement) PQCEncapsulate() (ciphertext, sharedSecret []byte, err error) {
	...
	return pqcrypto.KyberEncapsulate(p1.pqcKeypair.Kyber.PublicKey)
}
...
func (p1 *Phase1Enhancement) PQCDecapsulate(ciphertext []byte) ([]byte, error) {
	...
	return pqcrypto.KyberDecapsulate(ciphertext, p1.pqcKeypair.Kyber.SecretKey)
}
...
func LoadPhase1Config() Phase1Config {
	return Phase1Config{
		...
		PQCAlgorithm:           getenvStr("PHASE1_PQC_ALGORITHM", "hybrid"),
