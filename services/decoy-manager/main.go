...
var (
    ...
    // bandit stats
    tmplStats   = struct{ mu sync.Mutex; pulls map[string]int; wins map[string]int }{pulls: map[string]int{}, wins: map[string]int{}}

    // registry
    instances   = struct{ mu sync.Mutex; byID map[string]decoyInstance }{byID: map[string]decoyInstance{}}
)
...
func main() {
    ...
    if useDocker {
        ...
        if err != nil { log.Printf("[decoy-manager] docker client error: %v (fallback to local decoy)", err); useDocker = false } else { dockerCli = cli }
    }
    ...
    mux.HandleFunc("/spawn", func(w http.ResponseWriter, r *http.Request) {
        ...
        if useDocker && req.Kind == "http" {
            ...
            if err != nil { http.Error(w, "no free port", 500); return }
            ...
            if err != nil { log.Printf("spawn docker: %v", err); http.Error(w, "spawn error", 500); return }
        ...
        instances.mu.Lock(); instances.byID[id] = inst; instances.mu.Unlock()
    ...
    mux.HandleFunc("/analyze", func(w http.ResponseWriter, r *http.Request) {
        ...
        if path := os.Getenv("SANDBOX_WASM_PATH"); path != "" {
            if b, err := os.ReadFile(path); err == nil {
                fn := os.Getenv("SANDBOX_WASM_FUNC"); if fn=="" { fn = "analyze" }
                wr := sandbox.NewWASMRunner(b, fn, 2*time.Second)
                if s, err2 := wr.Run(r.Context(), req.Payload); err2 == nil { out += "\nWASM:" + s }
            }
        }
    ...
    mux.HandleFunc("/list", func(w http.ResponseWriter, r *http.Request) {
        instances.mu.Lock(); defer instances.mu.Unlock()
        var arr []decoyInstance
        for _, v := range instances.byID { arr = append(arr, v) }
    ...
    go func() {
        ...
        for range t.C {
            ...
            for _, inst := range instances.byID {
                if now.After(inst.ExpiresAt) { toStop = append(toStop, inst); delete(instances.byID, inst.ID) }
            }
            instances.mu.Unlock()
            for _, inst := range toStop { stopContainer(inst.DockerID) }
    ...
    reg.Register(mSpawn); reg.Register(mAnalyze); reg.Register(mRunning)
...
func chooseTemplate() decoyTemplate {
    ...
    tmplStats.mu.Lock(); defer tmplStats.mu.Unlock()
