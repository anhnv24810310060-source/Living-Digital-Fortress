...
func startQUICServer(addr string) error {
    ...
    if reg != nil { reg.Register(mQUICPathChange); reg.Register(mQUICDgramRecv); reg.Register(mQUICDgramSent); reg.Register(mQUICConnOpen); reg.Register(mQUICConnClose); reg.RegisterGauge(gQUICConns); reg.Register(mQUICPingRecv); reg.Register(mQUICPongSent) }
    go func() {
    ...
    go func() {
        for {
            ...
            if err != nil { continue }
            mQUICConnOpen.Inc(); gQUICConns.Set(gQUICConns.Get()+1)
            ...
            go func(c quic.Connection) {
                defer func(){ mQUICConnClose.Inc(); if g := gQUICConns.Get(); g>0 { gQUICConns.Set(g-1) } }()
                handleQUICConn(c)
            }(conn)
        }
    }()
...
func handleQUICConn(conn quic.Connection) {
    ...
    for {
        ...
        go func(st quic.Stream) {
            ...
            if err := json.Unmarshal(b, &env); err != nil { return }
            info, ok := chanReg.get(env.ChannelID)
            if !ok || time.Now().After(info.Expiry) { return }
            // forward to guardian
...
func handleQUICDatagrams(conn quic.Connection) {
    ...
    for {
        ...
        if isPingDatagram(b) {
            mQUICPingRecv.Inc()
            if err := conn.SendMessage([]byte(`{"pong":true}`)); err == nil { mQUICPongSent.Inc() }
            continue
        }
        if err := json.Unmarshal(b, &env); err != nil { continue }
        info, ok := chanReg.get(env.ChannelID)
        if !ok || time.Now().After(info.Expiry) { continue }
        guardian := fmt.Sprintf("http://127.0.0.1:%d/wch/recv", getenvInt("GUARDIAN_PORT", 9090))
        resp, err := httpPost(guardian, b)
        if err != nil { continue }
        if err := conn.SendMessage(resp); err == nil { mQUICDgramSent.Inc() }
    }
}
...
func remoteAddrString(conn quic.Connection) string {
    ...
    if ua, ok := ra.(*net.UDPAddr); ok { return ua.String() }
    return ra.String()
}
...
func generateRotatingTLSConfig() *tls.Config {
    ...
    go func(){
        ...
        for range t.C {
            cert, key := generateSelfSigned()
            if tlsCert, err := tls.X509KeyPair(cert, key); err == nil { currentCert.Store(&tlsCert) }
        }
    }()
    return &tls.Config{
